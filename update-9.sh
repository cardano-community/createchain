...
EPOCH=$1
export PATH=~/.cabal/bin-alpha:$PATH
ROOT=example
...
rm -f update-d && cardano-cli governance create-update-proposal \
            ...
            --cost-model-file costmdl.json --max-block-execution-units '(20000000000,62000000)'

# Create a transaction body containing the update proposal.

rm -f tx9u.txbody && cardano-cli transaction build-raw \
            --babbage-era \
            --fee 0 \
            --tx-in $TXID\
            --tx-out $(cat addresses/user1.addr)+$((${COINS_IN_INPUT} )) \
            --update-proposal-file update-d \
            --out-file tx9u.txbody
...
cardano-cli transaction sign \
            ...
            --testnet-magic 141 \
            --tx-body-file  tx9u.txbody \
            --out-file      tx9u.tx


cardano-cli transaction submit --tx-file tx9u.tx --testnet-magic 141
